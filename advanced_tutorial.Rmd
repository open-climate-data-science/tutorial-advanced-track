...
forecast_xts <- xts(forecast$fitted[1:length(forecast_dates)], order.by=forecast_dates)
```
...
```{r}
tbats_test = tbats(as.vector(temp_ts))
...
- We will begin clustering counties by reading in county data

- We consider the month of January
...
last_nclimk_raw <- last_nclimk_raw_jan
...
- Below we  get temparary cluster data for each county.

```{r}

numVarstoCluster<- nclimk_last%>%
                   select(prcp_cm,tavg_degf,tmax_degf)

head(numVarstoCluster)

```

- Now we compute k means for lots of values of k, 2 thru 12. And determine the average silhouette score:
...
- Now we plot to determine the highest shil. score: 
...
- This tells us 3 counties is the best! 
